...
import { getTransformInfo } from "../../logic/request";
//
const route = useRoute();
const id = computed(() => route?.query.id as string);

const data = ref(await getTransformInfo(id.value));
...
async function refetch() {
  ...
  //   if (resolved) resolved = `/@fs/${resolved.slice(8)}`;
  ...
  // }
  if (id.value) {
    data.value = await getTransformInfo(id.value);
  }
}
