...
import { getLoaderInfo } from '../../logic/request';

const data = ref(await getLoaderInfo());
...
onRefetch.on(async () => {
  data.value = await getLoaderInfo();
});
</script>
...
<template>
  <NavBar>
    ...
    <div class="text-sm font-mono my-auto">Loader Metrics</div>
    <div class="flex-auto" />
  </NavBar>
  <Container v-if="data" class="overflow-auto">
    <div
      class="mb-4 grid grid-cols-[1fr_max-content_max-content_max-content_max-content_max-content_1fr] mt-2 whitespace-nowrap text-sm font-mono children:(px-4 py-2 border-b border-main align-middle)"
    >
      <div />
